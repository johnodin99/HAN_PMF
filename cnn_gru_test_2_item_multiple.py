...
for get_experiment_result in range(FLAGS.get_result_times):
    for dataset in datasets:
        for seed in seeds:
            ...
            pmf_model_1 = pmf_model.PMF(R=R_zeros,
                                        U=None, V=None,
            ...
            model_item = CNN_GRU_module(output_dimesion=FLAGS.latent_size,
                                        vocab_size=FLAGS.vocab_size,
                                        dropout_rate=FLAGS.dropout_rate,
                                        emb_dim=FLAGS.embedding_size,
                                        gru_outdim=FLAGS.latent_size,
                                        maxlen_doc=maxlen_doc_item,
                                        maxlen_sent=maxlen_sent_item,
                                        nb_filters=FLAGS.nb_filters,
                                        init_W=None)
            item_sequence2D = index_to_input(item_sequence2D, maxlen_doc_item, maxlen_sent_item)
            item_weight = np.ones(len(item_sequence2D), dtype=float)

            for epoch in range(FLAGS.num_epochs):
                f1.write("Epoch:{}\n".format(epoch + 1))
                print('current epoch %s' % (epoch + 1))

                model_item.train(item_sequence2D, V, item_weight, seed)
                V = model_item.get_projection_layer(item_sequence2D)
                pmf_model_2 = pmf_model.PMF(R=R_zeros, U=U, V=V, latent_size=FLAGS.latent_size,
                                            iterations=FLAGS.pmf_iteration,
                                            lambda_alpha=FLAGS.pmf_lambda_u,
                                            lambda_beta=FLAGS.pmf_lambda_v,
                                            lr=FLAGS.pmf_learning_rate,
